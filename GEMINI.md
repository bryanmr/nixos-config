...
### Tauri Development
To support Tauri development, several packages have been added to `home.nix`. These include `webkitgtk`, `gtk3`, `libsoup`, `openssl`, `dbus`, `pkg-config`, `libappindicator-gtk3`, and `cargo-tauri` from the unstable channel. These packages provide the necessary libraries and tools for building Tauri applications.
